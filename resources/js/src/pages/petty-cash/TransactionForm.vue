...
export default {
  setup() {
    ...
    const fetchTransaction = async (id) => {
      loading.value = true
      try {
        const response = await axios.get(`/petty-cash/${id}`)
        ...
        form.value = {
          ...
          bs_date: formatDateForInput(transaction.bs_date),
          cash_in: parseFloat(transaction.cash_in) || 0,
          cash_out: parseFloat(transaction.cash_out) || 0,
          vat_percentage: parseFloat(transaction.vat_percentage) || 13,
          vat_amount: parseFloat(transaction.vat_amount) || 0,
          is_vat_included: Boolean(transaction.is_vat_included)
        }
      } catch (error) {
    ...
    const handleAmountChange = () => {
      if (form.value.cash_in > 0 && form.value.cash_out > 0) {
        Swal.fire({
          title: "Warning!",
          text: "You cannot have both cash in and cash out for a single transaction.",
          icon: "warning",
        })
    ...
    const calculateVat = () => {
      if (form.value.vat_percentage > 0 && form.value.cash_out > 0) {
        ...
        if (form.value.is_vat_included) {
          ...
          form.value.vat_amount = amount * (form.value.vat_percentage / 100)
        }
      } else {
    ...
    const submitForm = async () => {
      ...
      try {
        ...
        if (isEditMode.value) {
          await axios.put(`/petty-cash/${route.params.id}`, form.value)
          Swal.fire({
            title: "Success!",
          ...
          await axios.post("/petty-cash", form.value)
          Swal.fire({
            title: "Success!",
        ...
        Swal.fire({
          title: "Error!",
          text: error.response?.data?.message || "Failed to save transaction.",
          icon: "error",
        })
      } finally {
        loading.value = false
